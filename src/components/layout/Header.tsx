...
const Header: React.FC = () => {
  ...
  // Apply dark mode globally on <html>
  useEffect(() => {
    if (darkMode) {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          ...
          <nav className="hidden md:flex space-x-8 items-center">
            <Link to="/" className={`${isActive('/') ? 'text-purple-600 font-medium dark:text-purple-400' : 'text-gray-500 hover:text-purple-600 dark:text-gray-300 dark:hover:text-purple-400'} transition-colors duration-200`}>
              Home
            </Link>
            <Link to="/dashboard" className={`${isActive('/dashboard') ? 'text-purple-600 font-medium dark:text-purple-400' : 'text-gray-500 hover:text-purple-600 dark:text-gray-300 dark:hover:text-purple-400'} transition-colors duration-200`}>
              Dashboard
            </Link>
            <Link to="/profile" className={`${isActive('/profile') ? 'text-purple-600 font-medium dark:text-purple-400' : 'text-gray-500 hover:text-purple-600 dark:text-gray-300 dark:hover:text-purple-400'} transition-colors duration-200`}>
              Profile
            </Link>
      ...
      {isMenuOpen && (
        <div className="md:hidden dark:bg-gray-900">
          <div className="pt-2 pb-3 space-y-1">
            ...
            {/* Add the rest here like Dashboard, Profile, Sign In... */}
          </div>
        </div>
      )}
    </header>
